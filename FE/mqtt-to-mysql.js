...
// Cấu hình MQTT
const mqttOptions = {
  host: '192.168.0.102', // Địa chỉ broker MQTT (ví dụ: 'localhost' hoặc IP của broker)
  port: 1885,
  protocol: 'mqtt'
};
...
client.on('connect', () => {
  ...
  client.subscribe(['esp32/sensorData', 'esp32/led1', 'esp32/led2', 'esp32/led3', 'esp32/all_leds'], (err) => {
    if (err) {
      console.error('Lỗi khi đăng ký vào các topic MQTT: ', err);
    } else {
      console.log('Đã đăng ký vào các topic: esp32/sensorData, esp32/led1, esp32/led2, esp32/led3, esp32/all_leds');
    }
  });
});
...
client.on('message', (topic, message) => {
  if (topic === 'esp32/sensorData') {
  ...
  } else if (['esp32/led1', 'esp32/led2', 'esp32/led3'].includes(topic)) {
    ...
    const devices = ['led1', 'led2', 'led3']; // Danh sách tất cả các LED
